...
tmux new-window  -t obp:6  -n srvc-pipeline1    -d "tools/scripts/service-evidence-pipeline-run.sh --consumer-partition 0"        || exit 1
tmux new-window  -t obp:7  -n srvc-pipeline2    -d "tools/scripts/service-evidence-pipeline-run.sh --consumer-partition 1"        || exit 1

# applications
tmux new-window  -t obp:8  -n rest-app          -d "./go.sh 1"    || exit 1
tmux new-window  -t obp:9  -n evidence-app      -d "./go.sh 4"    || exit 1
