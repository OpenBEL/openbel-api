...
# run via: bash <(curl -s https://raw.githubusercontent.com/OpenBEL/openbel-api/master/bin/setup-prod-server.sh)
...
echo "Re-run docker-compose -f docker-compose-prod.yml build if you changed config.yml"
echo "Now run docker-compose -f docker-compose-prod.yml up to start the server"